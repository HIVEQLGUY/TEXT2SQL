...
## 37. 批准后发布路径改为无参数确定性晋级 2026-07-26

- 已将批准后的正常入口固化为 `warehouse-promote.cmd`：不传文件路径时，只扫描正式发布包，要求正式授权、`source_role: approved_shadow_result_for_formalization`、影子报告 `succeeded/finalized`、指纹一致且正式包与影子包有明确绑定。
- 影子验证开始前应同时预生成正式发布包和 SQL；用户批准只释放正式 `full` 执行门禁，不再由 AI 重新寻找文件、重写 SQL、重复做 `plan/verify` 或逐阶段手工执行。
- 自动路由对多个待执行候选、失败中的旧版本、缺少绑定和指纹不一致直接阻断；已完成版本幂等返回，Git `version_record_pending` 自动转入 `finalize`。多版本并行时仍可传显式影子 YAML 排查。
- 兼容历史正式包的旧 `artifacts.shadow_release` 若指向被阻断影子，只有在同目录找到唯一满足来源表、分区、目标表、粒度、主键、报告状态和指纹的成功影子时才允许修复路由；当前物流正式包已通过真实 `plan` 验证该路径，未执行写入。
- 发布器回归测试已从 21 项增加到 23 项并全部通过；清洗 Skill 源文件和当前已安装版本均已补充“正式包预生成、批准后 `warehouse-promote.cmd` 一次完成”的规则，Skill 校验通过。
