...
class Handler(SimpleHTTPRequestHandler):
    ...
    def handle_agent(self, payload: dict) -> None:
        config = load_database_config()
        timeline = Timeline()
        ...
        if not sql:
            ...
            plan_node = timeline.start("generate_sql", "理解需求并生成 SQL")
            generated_plan = plan_query(question, metadata)
            if generated_plan:
                sql = generated_plan.sql
                timeline.end(
                    plan_node,
                    summary=f"已生成：{generated_plan.title}",
                    details={
                        "title": generated_plan.title,
                        "confidence": generated_plan.confidence,
                        "assumptions": generated_plan.assumptions,
                        "sql": generated_plan.sql,
                    },
                )
            else:
            ...
            self.send_json(
                {
                    ...
                    "status": "blocked" if review.hard_blocks else "risk_pending",
                    "sql": sql_to_review,
                    "generated_plan": asdict(generated_plan) if generated_plan else None,
                    "hard_blocks": review.hard_blocks,
                    "risks": review.risks,
