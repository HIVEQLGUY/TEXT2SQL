...
FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_df__candidate__1_4_2;

SELECT throwIf(
    (SELECT count() FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_df__candidate__1_4_2) = 0,
    '正式快递单号粒度候选表为空'
);

SELECT throwIf(
    (SELECT count() FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_df__candidate__1_4_2)
...
SELECT throwIf(
    (SELECT countIf(tracking_no IS NULL OR tracking_no = '')
       FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_df__candidate__1_4_2) != 0,
    '正式快递单号粒度候选表存在空快递单号'
);
...
SELECT throwIf(
    (SELECT count() FROM system.columns
      WHERE database = 'youmei_sandbox'
        AND table = 'dwd_trade_order_logistics_tracking_no_df__candidate__1_4_2'
...
SELECT throwIf(
    ...
    OR (SELECT sum(package_count) FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_df__candidate__1_4_2)
       != (SELECT sum(package_count) FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_shadow_1_3_3)
    OR (SELECT sum(package_product_row_count) FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_df__candidate__1_4_2)
       != (SELECT sum(package_product_row_count) FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_shadow_1_3_3)
    OR (SELECT sum(sku_order_count) FROM youmei_sandbox.dwd_trade_order_logistics_tracking_no_df__candidate__1_4_2)
